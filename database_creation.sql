...
ALTER TABLE staff MODIFY role VARCHAR(255) NULL;



DELETE FROM complaint_resolution;
...
DELETE FROM room_clean; 

DELETE FROM cleaning_session;

UPDATE staff SET manager_id = NULL;

DELETE FROM staff WHERE manager_id = 6;
...
MODIFY reservation_date_time DATETIME DEFAULT CURRENT_TIMESTAMP;

SELECT * FROM complaint_category;



INSERT INTO complaint_category (category_code, category_name, severity) VALUES ('CS4', 'Noise', '1');

ALTER TABLE complaint_category 
MODIFY severity VARCHAR(255) DEFAULT '1';

select * from check_in;

-- Delete existing check-in record if not needed
DELETE FROM check_in WHERE reservation_id = 1; -- Adjust the condition as necessary

-- Delete existing check-out record if not needed
DELETE FROM check_out WHERE reservation_id = 1; -- Adjust the condition as necessary

-- Delete existing cleaning session record if not needed

select * from cleaning_session;

DELETE FROM cleaning_session WHERE date_of_clean = '2023-12-01'; -- Adjust the condition as necessary
...
DROP FOREIGN KEY complaint_ibfk_3;

ALTER TABLE complaint 
DROP COLUMN opened_by;

ALTER TABLE complaint 
ADD COLUMN opened_by VARCHAR(255);

INSERT INTO complaint (reservation_id, opened_date, category_code, description) VALUES (1, '2022-08-19', 'CS4', 'Loud noise from neighboring room');
INSERT INTO complaint (reservation_id, opened_date, category_code, description) VALUES (2, '2022-08-19', 'CS4', 'Loud noise from neighboring room');

INSERT INTO complaint (reservation_id, opened_date, category_code, description) VALUES (3, '2022-08-19', 'CS4', 'Loud noise from neighboring room');
INSERT INTO complaint (reservation_id, opened_date, category_code, description) VALUES (4, '2022-08-19', 'CS4', 'Loud noise from neighboring room');

INSERT INTO complaint_category (category_code, category_name, severity) VALUES ('CS6', 'Noise', '5');
...
INSERT INTO complaint_category (category_code, category_name, severity) VALUES ('CS9', 'Noise', '3');

INSERT INTO complaint (reservation_id, opened_date, category_code, description) VALUES (1, '2022-06-17', 'CS9', 'Loud noise from neighboring room');
...
INSERT INTO address (postcode, address_line1, address_line2, city, county) VALUES ('TS7 4AD', 'XA', 'YB', 'Sample City', 'Sample County');
INSERT INTO guest (guest_id, title, first_name, last_name, postcode) VALUES (1, 'Mr' 'John', 'Doe', 'TS3 5AD');

SELECT postcode FROM address WHERE postcode = 'TS4 4AD';
DELETE FROM address WHERE postcode = 'TS4 4AD';


DELETE FROM complaint WHERE reservation_id = 1;
DELETE FROM check_out WHERE reservation_id = 1; 
DELETE FROM check_in WHERE reservation_id = 1; -- Replace 1 with the actual reservation_id
DELETE FROM reservation WHERE guest_id = 1; -- Replace with the actual guest_id
DELETE FROM guest WHERE guest_id = 1; -- Replace with the actual guest_id

DELETE FROM reservation WHERE guest_id = 2; -- Replace with the actual guest_id
DELETE FROM guest WHERE guest_id = 2; -- Replace with the actual guest_id
--

-- Ensure the address exists before inserting guests
INSERT IGNORE INTO address (postcode, address_line1, address_line2, city, county) 
VALUES ('TS4 4AD', 'XA', 'YB', 'Sample City', 'Sample County');

-- Ensure that the guest is inserted with a valid postcode
INSERT IGNORE INTO guest (guest_id, title, first_name, last_name, phone_number, email, postcode) 
VALUES (1, 'Mr', 'John', 'Doe', '07123456789', 'john.doe@example.com', 'TS4 4AD');

-- Ensure that the rooms exist before assigning guests to them
INSERT IGNORE INTO room (room_number, room_type_code, bathroom_type_code, status, key_serial_number) 
VALUES (101, 'STD', 'SH', 'ACT', 'KEY101'),
       (102, 'STD', 'SH', 'ACT', 'KEY102');

-- Clean up logic in tearDown method to avoid foreign key constraint violations
DELETE FROM reservation WHERE guest_id = 1;  -- Delete reservations first
DELETE FROM guest WHERE guest_id = 1;         -- Then delete the guest

DELETE FROM reservation WHERE guest_id = 2;  -- Delete reservations first
DELETE FROM guest WHERE guest_id = 2;         -- Then delete the guest

-- First, delete any guests that reference the address
DELETE FROM guest WHERE postcode = 'TS4 4AD';

-- Now, delete the address
DELETE FROM address WHERE postcode = 'TS4 4AD';

DELETE FROM address WHERE postcode = 'TS4 4AD'; -- Finally delete the address
DELETE FROM room WHERE room_number IN (101, 102); -- Clean up rooms if necessary

--

-- Drop foreign key constraint from the guest table
ALTER TABLE guest DROP FOREIGN KEY guest_ibfk_2;

-- Drop foreign key constraint from the reservation table (if applicable)
ALTER TABLE reservation DROP FOREIGN KEY reservation_ibfk_1;

-- Drop foreign key constraint from the complaint table (if applicable)
ALTER TABLE complaint DROP FOREIGN KEY complaint_ibfk_1;

-- Add more ALTER TABLE statements as needed for other foreign keys



-- Drop foreign key constraint from the cleaning_session table
ALTER TABLE cleaning_session DROP FOREIGN KEY cleaning_session_ibfk_1;



-- Add more ALTER TABLE statements as needed for other foreign keys

-- Example: Deleting orphaned records (use with caution)
DELETE FROM guest WHERE postcode NOT IN (SELECT postcode FROM address);

-- Re-add foreign key constraint from the guest table to the address table
ALTER TABLE guest 
ADD CONSTRAINT guest_ibfk_2 
FOREIGN KEY (postcode) REFERENCES address(postcode) 
ON DELETE RESTRICT ON UPDATE CASCADE;

-- Re-add foreign key constraint from the cleaning_session table to the staff table
ALTER TABLE cleaning_session 
ADD CONSTRAINT cleaning_session_ibfk_1 
FOREIGN KEY (staff_id) REFERENCES staff(staff_id) 
ON DELETE RESTRICT ON UPDATE CASCADE;

-- Re-add foreign key constraint from the reservation table to the guest table (if applicable)
ALTER TABLE reservation 
ADD CONSTRAINT reservation_ibfk_1 
FOREIGN KEY (guest_id) REFERENCES guest(guest_id) 
ON DELETE RESTRICT ON UPDATE CASCADE;

-- Re-add foreign key constraint from the complaint table to the reservation table
ALTER TABLE complaint 
ADD CONSTRAINT complaint_ibfk_1 
FOREIGN KEY (reservation_id) REFERENCES reservation(reservation_id) 
ON DELETE RESTRICT ON UPDATE CASCADE;
...
WHERE 
    r1.reservation_id > r2.reservation_id AND  -- Assuming there's a unique ID column
    r1.guest_id = r2.guest_id AND 
    r1.room_number = r2.room_number;


ALTER TABLE reservation 
ADD CONSTRAINT unique_guest_room 
UNIQUE (guest_id, room_number);
...
-- Insert a reservation for guest_id 33 and room_number 101
INSERT INTO reservation (guest_id, room_number, reservation_staff_id, number_of_guests, start_of_stay, length_of_stay, reservation_date_time) 
VALUES (33, 101, 1, 2, '2023-10-01', 3, NOW());

-- Attempt to insert the same reservation again to trigger an SQLException
INSERT INTO reservation (guest_id, room_number, reservation_staff_id, number_of_guests, start_of_stay, length_of_stay, reservation_date_time) 
VALUES (33, 101, 1, 2, '2023-10-01', 3, NOW());

select * from reservation;

-- Attempt to insert a reservation with a non-existent guest_id
INSERT INTO reservation (guest_id, room_number, reservation_staff_id, number_of_guests, start_of_stay, length_of_stay, reservation_date_time) 
VALUES (999, 101, 1, 2, '2023-10-01', 3, NOW());